...
public class ExampleAccounts {
    ...
    ApplicationRunner bootstrapAdministrator(IdentityService identity,
        ...
        return arguments -> {
            if (!password.isBlank() && !identity.exists("admin@brunexa.com")) {
                identity.createAccount("admin", "admin@brunexa.com", "Administración Brunexa",
    ...
    private void createIfMissing(IdentityService identity, JdbcTemplate jdbc, PasswordEncoder encoder,
                                 TransactionTemplate transactions, String username, String email, String name,
                                 String password, String role) {
        if (identity.exists(email) || identity.usernameTaken(username)) return;
        transactions.executeWithoutResult(status -> {
            jdbc.update("INSERT INTO app_users (username, email, full_name, password_hash) VALUES (?, ?, ?, ?)",
                    username, email, name, encoder.encode(password));
